...
exports.zeroLayerFilter = zeroLayerFilter;
...
core_1.INITIAL_DICTIONARY.categories.forEach((cat) => {
    TRADE_KEYWORDS.push((0, core_1.normalizeText)(cat.name));
    cat.synonyms.forEach((syn) => {
        TRADE_KEYWORDS.push((0, core_1.normalizeText)(syn));
    });
});
/**
 ...
 */
function zeroLayerFilter(text) {
    if (!text || text.trim().length < 2)
        return false;
    const normalized = (0, core_1.normalizeText)(text);
    // 1. Shoshilinch/Favqulodda holat kalit so'zi bormi? (Har doim o'tadi)
    const isEmergencyMatch = EMERGENCY_KEYWORDS.some((kw) => normalized.includes((0, core_1.normalizeText)(kw)));
    if (isEmergencyMatch)
        return true;
    // 2. So'roq belgisi '?' bormi?
    const hasQuestionMark = text.includes('?');
    // 3. Savol so'zlaridan biri bormi?
    const hasQuestionWord = QUESTION_KEYWORDS.some((kw) => normalized.includes((0, core_1.normalizeText)(kw)));
...
//# sourceMappingURL=zeroLayerFilter.js.map