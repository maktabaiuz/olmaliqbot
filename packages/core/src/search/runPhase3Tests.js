...
async function runPhase3Tests() {
    ...
    assert('"karzinka oldida" -> "karzinka"', (0, dictionary_1.stripLandmarkSuffixes)('karzinka oldida') === 'karzinka');
    assert('"bozor orqasida" -> "bozor"', (0, dictionary_1.stripLandmarkSuffixes)('bozor orqasida') === 'bozor');
    assert('"корзинка возле" -> "корзинка"', (0, dictionary_1.stripLandmarkSuffixes)('корзинка возле') === 'корзинка');
    // --- 2. Database Seeding for Multi-City Testing ---
    console.log('\n--- 2. Ikki Shahar (Olmaliq & Chirchiq) Test Ma\'lumotlarini Yaratish ---');
    // Create Olmaliq City
    const olmaliq = await db_1.db.city.upsert({
        where: { slug: 'olmaliq-test' },
        update: {},
        create: { name: 'Olmaliq Test', slug: 'olmaliq-test', planType: 'ASOSCHI' },
    });
    // Create Chirchiq City
    const chirchiq = await db_1.db.city.upsert({
    ...
    const sobirOlmaliq = await db_1.db.listing.create({
        data: {
            cityId: olmaliq.id,
            categoryId: gazavikCat.id,
            name: 'Sobir Gazavik',
    ...
    const jamshidChirchiq = await db_1.db.listing.create({
        data: {
            cityId: chirchiq.id,
            categoryId: gazavikCat.id,
    ...
    const search1 = await (0, searchEngine_1.searchListings)({
        cityId: olmaliq.id,
    ...
    const searchServiceArea = await (0, searchEngine_1.searchListings)({
        cityId: olmaliq.id,
    ...
    const searchChirchiq = await (0, searchEngine_1.searchListings)({
        cityId: chirchiq.id,
        categoryName: 'gazavik',
    });
    ...
    const searchOlmaliqStrict = await (0, searchEngine_1.searchListings)({
        cityId: olmaliq.id,
        categoryName: 'gazavik',
    });
    ...
    assert(`Qidiruv ijro vaqti: ${search1?.executionTimeMs}ms (< 3000ms qoidasiga mos)`, (search1?.executionTimeMs || 0) < 3000);
    console.log('\n--- Real Guruh Javobi Misoli ---');
    console.log(search1?.formattedText);
    console.log('-------------------------------\n');
    // --- Cleanup Test Data ---
    await db_1.db.listing.deleteMany({ where: { cityId: { in: [olmaliq.id, chirchiq.id] } } });
    await db_1.db.landmark.deleteMany({ where: { cityId: { in: [olmaliq.id, chirchiq.id] } } });
    await db_1.db.city.deleteMany({ where: { id: { in: [olmaliq.id, chirchiq.id] } } });
    console.log('======================================================');
...
//# sourceMappingURL=runPhase3Tests.js.map