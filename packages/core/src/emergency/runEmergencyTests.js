...
function runEmergencyTests() {
    ...
    const categories = Object.keys(templates_1.EMERGENCY_TEMPLATES);
    assert('Jami 14 ta favqulodda shablon mavjud', categories.length === 14);
    categories.forEach((cat) => {
        const renderedLotin = (0, render_1.renderEmergencyTemplate)(cat, 'lotin');
        const renderedKirill = (0, render_1.renderEmergencyTemplate)(cat, 'kirill');
        const renderedRus = (0, render_1.renderEmergencyTemplate)(cat, 'rus');
    ...
    const level1Categories = categories.filter((c) => templates_1.EMERGENCY_TEMPLATES[c].level === 1);
    assert('1-darajada 10 ta hayotga xavf holati mavjud', level1Categories.length === 10);
    level1Categories.forEach((cat) => {
        const text = (0, render_1.renderEmergencyTemplate)(cat, 'lotin') || '';
    ...
    const renderedWithoutLocalNum = (0, render_1.renderEmergencyTemplate)('gas_leak', 'lotin', {});
    ...
    const renderedWithLocalNum = (0, render_1.renderEmergencyTemplate)('gas_leak', 'lotin', { mahalliy_gaz: '+998 70 612 04 04' });
...
runEmergencyTests();
//# sourceMappingURL=runEmergencyTests.js.map