...
const queryLoop_1 = require("./queryLoop");
let totalTests = 0;
let passedTests = 0;
let failedTests = 0;
...
async function runPhase5Tests() {
    ...
    const kafelCat = await db_1.db.category.upsert({
        where: { name: 'Kafelchi' },
        update: { synonyms: ['kafelchi', 'plitkachi'] },
        create: { name: 'Kafelchi', synonyms: ['kafelchi', 'plitkachi'] },
    });
    const landmarkOlmaliq = await db_1.db.landmark.create({
    ...
    const clusters = await (0, queryLoop_1.clusterUnresolvedQueries)(olmaliq.id);
    ...
    const alisherListing = await db_1.db.listing.create({
        data: {
            cityId: olmaliq.id,
    ...
    const mockSendFn = async (userId, text) => {
        dispatchedMessages.push({ userId: userId.toString(), text });
        return true;
    };
    // Trigger Notification Loop for category "kafelchi"
    const notifResult = await (0, queryLoop_1.notifyUsersOnNewListingAdded)({
        cityId: olmaliq.id,
    ...
    const secondNotifResult = await (0, queryLoop_1.notifyUsersOnNewListingAdded)({
        cityId: olmaliq.id,
        listingId: alisherListing.id,
        categoryName: 'kafelchi',
        sendNotificationFn: mockSendFn,
    ...
    const chirchiqPendingLogs = await db_1.db.queryLog.findMany({
    ...
    // Cleanup Test Data
    await db_1.db.queryLog.deleteMany({ where: { cityId: { in: [olmaliq.id, chirchiq.id] } } });
...
//# sourceMappingURL=runPhase5Tests.js.map