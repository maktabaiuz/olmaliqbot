...
exports.clusterUnresolvedQueries = clusterUnresolvedQueries;
exports.notifyUsersOnNewListingAdded = notifyUsersOnNewListingAdded;
 ...
 */
async function clusterUnresolvedQueries(cityId) {
    if (!cityId)
        return [];
    // 1. Fetch unresolved query logs for the city
    const logs = await db_1.db.queryLog.findMany({
        where: {
            cityId,
            isResolved: false,
        },
    ...
    const existingCategories = await db_1.db.category.findMany();
    const clustersMap = new Map();
    for (const log of logs) {
        const rawCat = log.categoryName || log.rawMessage;
        const normalized = (0, transliteration_1.normalizeText)(rawCat);
        // Normalize trade names (kafelchi, plitkachi, kafel ustasi -> kafelchi)
        let canonical = normalized;
        if (/kafel|plitka/.test(normalized))
            canonical = 'kafelchi';
        else if (/gazovik|gazavik|gaz ustasi/.test(normalized))
            canonical = 'gazavik';
        else if (/santexnik|quvur|suv ustasi/.test(normalized))
            canonical = 'santexnik';
        else if (/elektrik|svet ustasi/.test(normalized))
            canonical = 'elektrik';
        // Match with existing categories
        const matchedCat = existingCategories.find((cat) => cat.name.toLowerCase() === canonical ||
 ...
 */
async function notifyUsersOnNewListingAdded(options) {
    ...
    const listing = await db_1.db.listing.findUnique({
        where: { id: listingId },
    ...
    const normCategory = (0, transliteration_1.normalizeText)(categoryName);
    let canonicalTrade = normCategory;
    if (/kafel|plitka/.test(normCategory))
        canonicalTrade = 'kafelchi';
    else if (/gazovik|gazavik|gaz ustasi/.test(normCategory))
        canonicalTrade = 'gazavik';
    else if (/santexnik|quvur|suv ustasi/.test(normCategory))
        canonicalTrade = 'santexnik';
    ...
    const pendingLogs = await db_1.db.queryLog.findMany({
        where: {
            cityId,
            isResolved: false,
            notifiedAt: null,
            OR: [
                ...
                ...searchTerms.map((term) => ({
                    rawMessage: { contains: term, mode: 'insensitive' },
                })),
            ],
        },
    });
